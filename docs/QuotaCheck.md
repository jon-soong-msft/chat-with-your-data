...
> **For Global Standard gpt-5.6-sol, ensure at least 50k tokens of capacity post-deployment for optimal performance.**
...
```
gpt-5.6-sol:50, text-embedding-3-small:100
```
### 📌 Default Regions:
```
...
✔️ Check specific model(s) in default regions:
  ```
  ./quota_check_params.sh --models gpt-5.6-sol:50,text-embedding-3-small:100
  ```
...
```
./quota_check_params.sh --models gpt-5.6-sol:50 --regions eastus2,japaneast
```
✔️ All parameters combined:
```
./quota_check_params.sh --models gpt-5.6-sol:50,text-embedding-3-small:100 --regions eastus2,japaneast --verbose
```
