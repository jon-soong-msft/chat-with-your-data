...
- **Azure OpenAI gpt-5.6-sol model**: The solution deploys the Azure OpenAI gpt-5.6-sol model for advanced natural language processing. This model handles complex legal queries and produces detailed, contextually appropriate responses.
...
The gpt-5.6-sol chat model is selected at deployment time through the `azd` environment, so no runtime change is required.
